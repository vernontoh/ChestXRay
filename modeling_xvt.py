...
class XvtLayer(nn.Module):
    def __init__(
        ...
        self.output = XvtOutput(embed_dim, mlp_ratio)
...
class XvtBlock(nn.Module):
    def __init__(self, config, block_idx):
        ...
        self.embedding = XvtConvEmbeddings(
            patch_size=config.patch_sizes[self.block_idx], 
            num_channels=config.num_channels if self.block_idx == 0 else config.embed_dim[self.block_idx-1],
            embed_dim=config.embed_dim[self.block_idx], 
            stride=config.patch_strides[self.block_idx], 
            padding=config.patch_padding[self.block_idx]
        )
        ...
        self.layers = nn.Sequential(
            *[
                XvtLayer(
                    embed_dim=config.embed_dim[self.block_idx],
                    kernel_size=config.kernel_qkv[self.block_idx],
                    padding=config.padding_qkv[self.block_idx],
                    stride=config.stride_qkv[self.block_idx],
                    mlp_ratio=config.mlp_ratio[self.block_idx],
                    num_heads=config.num_heads[self.block_idx]
                )
            for _ in range(config.depth[self.block_idx])
            ]
        )
...
class XvtEncoder(nn.Module):
    def __init__(self, config):
        ...
        for block_idx in range(len(config.depth)):
            self.blocks.append(XvtBlock(config, block_idx=block_idx))

    def forward(self, pixel_values):
        hidden_state = pixel_values

        for block_module in self.blocks:
            hidden_state = block_module(hidden_state)
...
class XvtForImageClassification(nn.Module):
    def __init__(self, config):
        ...
        # classification head
        self.classifier = nn.Linear(config.embed_dim[-1], config.num_labels)
